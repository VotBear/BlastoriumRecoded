...
class Tilelist {
	...
	public:
		...
		void DrawTile(int id,double x,double y,shared_ptr<sf::RenderWindow> targetwin);
		...
		void DrawTile(int id,double x,double y,int tra,shared_ptr<sf::RenderWindow> targetwin); 
		...
		void DrawTile(int id,double x,double y,double xsc,double ysc,shared_ptr<sf::RenderWindow> targetwin); 
		...
		void SetScale(int id,double xsc,double ysc);
			...
			////Sets tile [id]'s transparency (0..255, transparent to solid)
		void SetColor(int id,double tra);
		
			////Modifies tile [id]'s texture rectangle
		...
		void Rotate (int id,double amt);
			...
			////sets rotation of tile [id] (this sets it to exactly [amt] amount, unlike rotate)
